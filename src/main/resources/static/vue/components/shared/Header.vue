            ...
            <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                ...
                <ul class="nav navbar-nav navbar-right">
                    <li v-if="auth.username" class="dropdown">
                        <router-link :to="{path: '/user/' + auth.username}" class="dropdown-toggle navbar-img"
                                     data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">
                            {{auth.username}}
                            <img src="http://placehold.it/150x150" class="img-circle" alt="Profile Image"/>
                        </router-link>
                        <ul class="dropdown-menu">
                            <router-link tag="li" :to="{path: '/user/' + auth.username + '/profile'}"><a>Profile</a>
                            </router-link>
                            <router-link tag="li" :to="{path: '/user/' + auth.username + '/inbox'}"><a>Inbox</a>
                            </router-link>
                            <li role="separator" class="divider"></li>
                            <router-link tag="li" :to="{path: '/user/' + auth.username + '/settings'}"><a>Settings</a>
                            </router-link>
